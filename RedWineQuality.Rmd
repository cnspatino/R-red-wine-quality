...
> Above is a correlation matrix of the continuous variables of the dataset. Some strong correlations that were expected are fixed acidity vs. citric acid, fixed acidity vs. pH, citric acid vs. pH, and citric acid vs. volatile acidity. In addition, free sulfur dioxide and total sulfur dioxide have a strong correlation which makes sense because total sulfur dioxide is the total of free and bound sulfur dioxide. Surpringly, sulphate concentration is not correlated with free or total sulfur dioxide in this dataset; I expected these to be correlated because sulphates are a wine additive that can contribute to sulfur dioxide gas levels. Density appears to be moderately correlated with alcohol and fixed acidity.
...
> Higher quality wines tend to have higher alcohol contents. The group with quality 5 is the exception to this trend, and has the highest range and max value for alcohol content. 
...
```{r}
ggplot(aes(x=residual.sugar, y=density), data=wine) + geom_point(alpha=1/8)
```

```{r}
ggplot(aes(x=log(residual.sugar), y=density), data=subset(wine, residual.sugar<4)) +
  geom_jitter(alpha=1/4) +
  geom_smooth(method='lm')
```

> Residual sugars have the opposite effect on density as alcohol does. From the plot directly above, it appears to be a slight positive linear relationship when taking the log of residual sugar.
